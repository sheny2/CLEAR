...
CLEAR_all <- function(site_data_list,
                      method      = c("logistic", "knn"),
                      K_comp      = 3,
                      n_0         = 10000,
                      inf_factor  = 2,
                      k_neighbors = 15) {
  ...
  for (h in 1:H) {
    ...
    fit  <- Mclust(df_h, G = K_comp, modelNames = "VVV", verbose = FALSE)
    if (is.null(fit)) {
      warning(sprintf("Site %d: VVV model with G=%d failed, falling back to BIC selection over G=1:%d", 
                      h, K_comp, K_comp))
      fit <- Mclust(df_h, G = 1:K_comp, verbose = FALSE)
    }
    if (is.null(fit)) {
      stop(sprintf("Site %d: Mclust failed for both VVV and BIC fallback. Check data for near-zero variance or too few rows.", h))
    }
  ...
  # --- Step 3 & 4: IPW and EDA stats ---
  q_probs      <- c(0.05, 0.25, 0.5, 0.75, 0.95)
  results_list <- list()
  
  # build quadratic formula once outside loop (logistic only)
  if (method == "logistic") {
    quad_terms <- paste0("I(", var_names, "^2)", collapse = " + ")
    poly_form  <- as.formula(paste("~ .^2 +", quad_terms))
  }
  ...
  for (h in 1:H) {
    ...
    if (method == "logistic") {
      ...
      if (!glm_fit$converged)
        warning(sprintf("GLM did not converge for Site %d", h))
    ...
    names(site_stats$Mean)              <- names(site_stats$Variance) <- var_names
    rownames(site_stats$Quantiles)      <- var_names
    colnames(site_stats$Quantiles)      <- c("5%", "25%", "50%", "75%", "95%")
    rownames(site_stats$Covariance)     <- var_names
    colnames(site_stats$Covariance)     <- var_names
    ...
    for (v in 1:d) {
      ...
      w_ord <- w_norm[ord]
      cum_w <- cumsum(w_ord)
      site_stats$Quantiles[v, ] <- sapply(q_probs, function(p)
        x_ord[which(cum_w >= p)[1]])
    }
