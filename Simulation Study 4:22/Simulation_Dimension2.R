...
cat(sprintf("Starting %d total simulation tasks for varying Dimensions...\n", total_tasks))
...
sim_results <- foreach(
  ...
  current_d <- sim_grid$Dimension[i]
  sim <- sim_grid$Simulation[i]
  
  site_data_list <- generate_scalable_dgp(M = 5, d = current_d)
  truth_site <- calc_empirical_truth(site_data_list[[target_site]])
  
  est_site <- tryCatch({
    res <- CLEAR(site_data_list, K_comp = fixed_k, n_0 = fixed_n0, inf_factor = fixed_inf)
    res[[paste0("Site_", target_site)]]
  }, warning = function(w) {
    res <- suppressWarnings(CLEAR(site_data_list, K_comp = fixed_k, n_0 = fixed_n0, inf_factor = fixed_inf))
    res[[paste0("Site_", target_site)]]
  }, error = function(e) {
    return(NULL) 
  })
  ...
  if (!is.null(est_site) && !is.null(est_site$Mean)) {
    ...
    df_out <- rbind(df_out, data.frame(Metric = "Mean", Variable = 1:current_d, Est = est_site$Mean, Truth = truth_site$Mean))
    df_out <- rbind(df_out, data.frame(Metric = "Variance", Variable = 1:current_d, Est = est_site$Variance, Truth = truth_site$Variance))
    
    q_names <- c("Quantile_05", "Quantile_25", "Quantile_50", "Quantile_75", "Quantile_95")
    for(j in 1:5) {
      df_out <- rbind(df_out, data.frame(Metric = q_names[j], Variable = 1:current_d, Est = est_site$Quantiles[, j], Truth = truth_site$Quantiles[, j]))
    }
    
    df_out <- rbind(df_out, data.frame(Metric = "Covariance", Variable = 1:(current_d^2), Est = as.vector(est_site$Covariance), Truth = as.vector(truth_site$Covariance)))
    
    df_out$Dimension <- current_d
...
saveRDS(sim_results, "Results/Simulation_Dimension_Results.rds")

summary_metrics <- sim_results %>%
  group_by(Dimension, Metric, Variable) %>%
  summarize(
...
p_bias <- ggplot(summary_metrics, aes(x = Dimension, y = Avg_Bias, color = Metric, group = Metric)) +
...
p_var <- ggplot(summary_metrics, aes(x = Dimension, y = Avg_Variance, color = Metric, group = Metric)) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  scale_color_brewer(palette = "Set2") + 
  scale_y_log10() + 
...
print(combined_plot)
ggsave("Results/Simulation_Plot_Dimension.png", combined_plot, width = 12, height = 6, dpi = 300)